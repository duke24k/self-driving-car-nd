...
def telemetry(sid, data):
    if data:
        ...
        resized = cv2.resize( np.asarray(image), (320, 220))
        image_array = np.asarray(resized)
        print(np.mean(image_array))

#        image_array = (image_array / 255. - .5).astype(np.float32)

#        image = convert_gray_with_adapthist_debug(image_array)

#        image = (image[0] - .5).astype(np.float32)



 #       image_array = image.reshape(image.shape + (1,))

        image_array = (image_array / 255. - .5).astype(np.float32)

        image_array = image_array[50:190, 0:320]
